...
bool AFLCoverage::runOnModule(Module &M)
{
      ...
      do
      {
        ...
        for (uint32_t j = 0; j < countto; j++)
        {
          ...
          if(countto > 1){
            ...
            one_edge_new->edge_id = afl_global_edge_id;
            one_edge_new->next = NULL;
            if (edge_now->subedge == NULL)
            {
              edge_now->subedge = one_edge_new;
          ...
          if(BranchInst *BI = dyn_cast<BranchInst>(origBB->getTerminator())){
            if(BI->isConditional()){
  ...
  while(edge_now != NULL)
  {
    ...
    {
      fprintf(bb_file, "%u\n", one_edge_now->edge_id);
      one_edge_now = one_edge_now->next;
    }
